/**
 * Survey Logic & State Management - Updated with Validation Fixes
 */
...
// Selection Handlers (แก้ไขใหม่ให้ล้างข้อมูลช่องที่ไม่ได้เลือก)
function selectSingle(element, hasInput = false) {
  ...
  parent.querySelectorAll(".option-item").forEach(item => {
    ...
    // ถ้าข้อไหนไม่ได้ถูกเลือก ให้เคลียร์ข้อความในช่อง input ทิ้ง
...
function selectMultiple(element, maxLimit = 99) {
  ...
  // ถ้าเอาติ๊กออก ให้ล้างข้อความในช่อง input ทิ้งด้วย
...
// Validate Step (แก้ไขใหม่ให้ตรวจสอบ input เฉพาะข้อที่ถูกเลือก)
function validateStep(stepNumber) {
  ...
  for (let list of requiredLists) {
    ...
    // 1. ตรวจสอบว่าได้กดเลือกตัวเลือกหรือยัง
    if (selectedItems.length === 0) {
      alert("กรุณาเลือกคำตอบให้ครบถ้วนเพื่อไปต่อครับ");
      return false;
    }

    // 2. ตรวจสอบช่องกรอกข้อความ (เช็คเฉพาะตัวเลือกที่ 'ถูกกดเลือก' เท่านั้น)
    for (let item of selectedItems) {
      const input = item.querySelector('.inline-input');
      // ถ้ามีช่องให้กรอก และช่องนั้นว่างเปล่า
      if (input && input.value.trim() === '') {
        alert("กรุณาระบุข้อมูลเพิ่มเติมในตัวเลือกที่คุณเลือกด้วยครับ");
        input.focus(); // เด้งไปโฟกัสที่ช่องให้พิมพ์
        return false;
      }
    }
